...
Scope: Environment setup and build pipeline (Phase 0) + mDNS discovery (Phase 1). Connection, transport, stability, failure, and stress phases deferred to EP-002.
...
| Connection (WebSocket) | ⏭️ Skipped | Deferred to EP-002 |
| Ping/Pong RTT | ⏭️ Skipped | Deferred to EP-002 |
...
2. **No connection testing.** WebSocket connection, ping/pong, and RTT measurement not tested.
3. **No failure scenario testing.** Wi-Fi drop, agent kill, firewall block not tested.
4. **No stress testing.** 100-ping burst and 15-minute idle not tested.
5. **Single device pair.** Only tested with this specific desktop + Android phone combination.
6. **Temporary logging added.** INFO-level lifecycle logs in `MdnsDiscoveryProvider.kt` should be removed or demoted to DEBUG before production.
...
1. **Connection validation.** Complete Phase 2 (WebSocket connect, ping/pong, RTT measurement).
2. **Discovery repeatability.** Run 10 discovery scans, record latency distribution.
3. **Logging cleanup.** Remove or demote temporary INFO logs added in EP-001.
4. **UI polish.** Show discovered device IP/port in the device list.
5. **Desktop-side connection logging.** Log `New connection from <ip>:<port>` on the Rust agent.
6. **APK signing.** Generate a signed release APK for side-loading outside Android Studio.
...
If connection (Phase 2) reveals issues with WebSocket handshake or protocol, EP-002 may need to revisit the transport layer. Do not pre-emptively change the protocol.
